...
def output_tables() -> object:
    ...
    nuclei_table = PrettyTable(["Template", "Template-Id", "Host" ,"Type", "severity"])

    # to add data in file
    for table in range(len(json_data)):
        nuclei_table.add_row([json_data[table]['template'], json_data[table]['template-id'], json_data[table]['host'], json_data[table]['type'], json_data[table]["info"]['severity']])
